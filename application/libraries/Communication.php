...
class Communication {
   ...
   */
  public function send_confirmation_email($arr_param) {
    if (empty($arr_param)) {
      return FALSE;
    }
...
//    $mail->AddAddress('sharon@zoomgroup.com');
    ...
    //$mail->AddAttachment($arr_param['attach']);
   if($arr_param['attach'] != ''){
...
//----------------------------------------------------------------------------
   ...
   * @since       0.0.1             
   * @access      public
   * @param       array      $arr_param       Various values required for sending the mail without attachment
   * @return      boolean                     TRUE if mail has been sent successfully otherwise FALSE
   */
